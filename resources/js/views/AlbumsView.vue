...
<template>
    <div>
        ...
        <div v-if="albumsStore.loading && !albumsStore.hasAlbums" class="text-center py-12">
            <p class="text-surface-400">Loading albums...</p>
        </div>
        ...
        <div v-else-if="!albumsStore.hasAlbums" class="text-center py-12">
            <p class="text-surface-400">No albums found</p>
        </div>
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            ...
            >
                ...
                <p class="text-surface-400 text-sm truncate">{{ album.artist_name ?? 'Unknown Artist' }}</p>
                <p v-if="album.year" class="text-surface-500 text-xs mt-1">
                    {{ album.year }}
                </p>
            </RouterLink>
        </div>
        ...
        <div v-if="albumsStore.loading && albumsStore.hasAlbums" class="mt-4 text-center text-surface-400 text-sm">
            Loading...
        </div>
    </div>
</template>
