...
onMounted(() => {
    songsStore.fetchSongs({ incomplete: true });
});
...
function handleSongUpdated(updatedSong: Song, index: number): void {
    // If song is no longer incomplete, remove from list
    const isStillIncomplete =
        !updatedSong.album_id ||
        !updatedSong.artist_id ||
        !updatedSong.artist_name ||
        updatedSong.artist_name === 'Unknown' ||
        updatedSong.artist_name === 'Unknown Artist';

    if (isStillIncomplete) {
        songsStore.updateSongInList(updatedSong, index);
    } else {
        // Remove from list since it's now complete
...
function handleBulkUpdated(_updatedSongs: Song[]): void {
    // Re-fetch the incomplete songs list since some may now be complete
    songsStore.fetchSongs({ incomplete: true });
    clearSelection();
...
<template>
    <div>
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-3xl font-bold text-surface-50">Incomplete Songs</h1>
                <p class="text-surface-400 mt-1">Songs missing album or artist metadata</p>
            </div>
        ...
        <div v-else-if="!songsStore.hasSongs" class="text-center py-12">
            ...
            <p class="text-surface-400 text-lg">All songs have complete metadata!</p>
        </div>
