...
function handleSongUpdated(updatedSong: Song, index: number): void {
    artistsStore.updateSongInArtist(updatedSong, index);
}
</script>

<template>
    <div>
        <div v-if="artistsStore.loading && !artistsStore.currentArtist" class="text-center py-12 bg-surface-800/50 rounded-lg" role="status">
            ...
            <span class="sr-only">Loading artist...</span>
        </div>
